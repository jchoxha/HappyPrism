...
class CanvasManager {
    ...
    drawNodeDetails(node) {
        ...
        this.ctx.fillStyle = "black";
        this.ctx.fillText(`ID: ${node.id}`, 10, y);y+=20;
        this.ctx.fillText(`X: ${node.x}`, 10, y);y+=20;
        this.ctx.fillText(`Y: ${node.y}`, 10, y);y+=20;
        this.ctx.fillText(`Size: ${node.size}`, 10, y);y+=20;
        this.ctx.fillText(`Shape: ${node.shapeType.name}`, 10, y);y+=20;
        this.ctx.fillText(`vx: ${node.vx}`, 10, y);y+=20;
        this.ctx.fillText(`vy: ${node.vy}`, 10, y);y+=20;
        this.ctx.fillText(`angle: ${node.angle}`, 10, y);y+=20;
        this.ctx.fillText(`intendedAngle: ${node.intendedAngle}`, 10, y);y+=20;
        this.ctx.fillText(`refAngle: ${node.refAngle}`, 10, y);y+=20;
        this.ctx.fillText(`Dragging: ${node.dragging}`, 10, y);y+=20;
        this.ctx.fillText(`dragOffsetX: ${node.dragOffsetX}`, 10, y);y+=20;
        this.ctx.fillText(`dragOffsetY: ${node.dragOffsetY}`, 10, y);y+=20;
        //~~~~~
